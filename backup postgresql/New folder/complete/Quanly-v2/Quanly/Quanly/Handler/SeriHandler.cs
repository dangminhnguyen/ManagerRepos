    ...
    class SeriHandler
    {
        ...
        public List<Model.Seri> layds(string mamay,string mamodel)
        {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT * from seri where (mamay=@mamay or @mamay='') and (mamodel=@mamodel or @mamodel='')";
                    cmd.Parameters.AddWithValue("mamay", mamay);
                    cmd.Parameters.AddWithValue("mamodel", mamodel);
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
        ...
        public int them(string ma, string ten, string mamay,string mamodel,string khachhang,DateTime ngaylapdat)
        {
            int serikey1 = 0;
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand())
                {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandText = "INSERT INTO seri (ma,ten,mamay,mamodel,khachhang,hientrang,ngaylapdat) VALUES (@ma,@ten,@mamay,@mamodel,@khachhang,'0',@ngaylapdat) RETURNING pk";
                    cmd.Parameters.AddWithValue("ma", ma);
                    cmd.Parameters.AddWithValue("ten", ten);
                    cmd.Parameters.AddWithValue("mamay", mamay);
                    cmd.Parameters.AddWithValue("mamodel", mamodel);
                    ...
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
                        {
                            serikey1 = reader.GetInt32(reader.GetOrdinal("pk"));
                        }
                }
            }
