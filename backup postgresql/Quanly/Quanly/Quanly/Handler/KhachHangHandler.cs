...
namespace Quanly.Handler
{
    class KhachHangHandler
    {
        ...
        public List<Model.KhachHang> tracuu(string ten)
        {
            List<Model.KhachHang> list = new List<Model.KhachHang>();
            using (var conn = new NpgsqlConnection(connString))
            {
                conn.Open();
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT * from khachhang where (ten=@ten or @ten='')";
                    cmd.Parameters.AddWithValue("ten", ten);
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
                        {
                            Model.KhachHang s = new Model.KhachHang();
