...
namespace Quanly.Handler
{
    class ThongkeLKHandler
    {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                    while (reader.Read())
                    {
                        ...
                        thongkelinhkien tk = new thongkelinhkien(keyseri, ma,thoigian,tinhtrang, maymoi,vitrimoi, mayve,xuatsu,ttrangmayve);
                        ls.Add(tk);
                    }
            }
            return ls;
        }
        public List<thongkelinhkien> layds1(int ks, string ma)
        {
            List<thongkelinhkien> ls = new List<thongkelinhkien>();
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var cmd = new NpgsqlCommand())
                {
                    ...
                    cmd.CommandText = "SELECT * from thongkelinhkien where (keyseri=@keyseri or @keyseri=0) and (ma=@ma or @ma='')";
                    cmd.Parameters.AddWithValue("ma", ma);
                    cmd.Parameters.AddWithValue("keyseri", ks);
                    ...
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
                        {
                            ...
                            thongkelinhkien tk = new thongkelinhkien(keyseri, ma1,thoigian, tinhtrang, maymoi, vitrimoi, mayve, xuatsu, ttrangmayve);
                            ls.Add(tk);
                        }
                }
        ...
        public void them(int keyseri, string ma, string tinhtrang, string maymoi,string vitrimoi, string mayve,string xuatsu, string ttrangmayve,DateTime thoigian)
        {
            using (var conn = new NpgsqlConnection(connString))
            {
                conn.Open();
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "INSERT INTO thongkelinhkien (keyseri,ma,tinhtrang,thoigian,mayhientai,vitrimoi,maythaythe,xuatsu,tinhtrangthaythe) VALUES (@keyseri,@ma,@tinhtrang,@thoigian,@mayhientai,@vitrimoi,@maythaythe,@xuatsu,@tinhtrangthaythe)";
                    cmd.Parameters.AddWithValue("ma", ma);
                    cmd.Parameters.AddWithValue("keyseri", keyseri);
        ...
        public thongkelinhkien traCuuGanNhat(int pk, string malk)
        {
            thongkelinhkien r = new thongkelinhkien();
            using (var conn = new NpgsqlConnection(connString))
            {
                conn.Open();
                // Retrieve all rows
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT * from thongkelinhkien where keyseri=@pk and ma=@ma " +
                        "and thoigian=(Select max(thoigian) from thongkelinhkien where keyseri=@pk and ma=@ma) ";
                    cmd.Parameters.AddWithValue("pk", pk);
                    cmd.Parameters.AddWithValue("ma", malk);
                    ...
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
                        {
                            ...
                            string xuatsu = reader.GetString(reader.GetOrdinal("xuatsu"));
                            DateTime thoigian = reader.GetDateTime(reader.GetOrdinal("thoigian"));
                            r = new thongkelinhkien(keyseri, ma, thoigian, tinhtrang, maymoi, vitrimoi, mayve, xuatsu, ttrangmayve);
                        }
                }
            }
            return r;
